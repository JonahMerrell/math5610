...
from _mymodules import matrix_solve_conjugate_gradient, matrix_solve_jacobian, gen_sqr_diagdom_matrix
...
def matrix_jacobian_conjugate_gradient_compare():
    for i in [10,50,100,200]:
        ...
        starttime = time.time()
        solution, count = matrix_solve_conjugate_gradient(matrix, vector, 0.0001, 10000, getIterCount=True)
        print(str(i)+ "x" + str(i) + " Conjugate Gradient: Time: " + str(time.time() - starttime) + ", Iteration Count: " + str(count))

matrix_jacobian_conjugate_gradient_compare()
