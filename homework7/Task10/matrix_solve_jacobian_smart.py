...
def matrix_solve_jacobian(matrix,vector_b,tol,max_iter,getIterCount=False):
    ...
    print(alpha)
    print(matrix_mod)
    #alpha = max(0,alpha - max([matrix_mod[i][i] for i in range(len(matrix_mod))]))

    print(max([matrix_mod[i][i] for i in range(len(matrix_mod))]))
...
print(matrix_solve_jacobian(matrix_example,vector_example,0.00001,10000))
